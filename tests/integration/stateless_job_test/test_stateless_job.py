...
def test__stop_start_tasks_when_mesos_main_down_and_jobmgr_restarts(
        stateless_job, mesos_main, jobmgr
):
    """
    1. Create stateless job.
    2. Wait for job state RUNNING.
    ...
    5. Start mesos main and wait for the instances to be stopped.
    6. Start the same subset of instances when mesos main is down.
    7. Restart job manager.
    8. Start mesos main and wait for the instances to transit to RUNNING.
    9. Stop the job when mesos main is down.
    10. Restart job manager.
    11. Start mesos main and wait for the job to terminate
    """
    stateless_job.create()
    stateless_job.wait_for_state(goal_state="RUNNING")
